...
<p>您可以通过修改daemon配置文件<code v-pre>/etc/docker/daemon.json</code>来使用加速器</p>
...
<h2 id="run做了些什么" tabindex="-1"><a class="header-anchor" href="#run做了些什么" aria-hidden="true">#</a> run做了些什么</h2>
<ol>
...
<h2 id="为什么docker比虚拟机快" tabindex="-1"><a class="header-anchor" href="#为什么docker比虚拟机快" aria-hidden="true">#</a> 为什么docker比虚拟机快</h2>
<p>因为docker不需要像hypervisor（虚拟机）实现硬件实例化，运行在docker容器上的程序直接使用的都是实例物理机的硬件资源，因此在CPU、内存利用率上docker更有效率</p>
<h2 id="hello-word案例" tabindex="-1"><a class="header-anchor" href="#hello-word案例" aria-hidden="true">#</a> hello-word案例</h2>
