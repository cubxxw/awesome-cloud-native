...
<h3 id="传统虚拟机和容器的对比" tabindex="-1"><a class="header-anchor" href="#传统虚拟机和容器的对比" aria-hidden="true">#</a> 传统虚拟机和容器的对比</h3>
<ol>
<li>传统的VMware从开始到运行，很慢，而且内存很大
<ul>
<li>资源占用多</li>
<li>冗余步骤多</li>
<li>启动慢</li>
...
<ul>
<li>docker容器是在操作系统层面实现虚拟化，直接复用本地主机的操作系统，容器之间相互隔离，进程互不影响。</li>
<li>传统虚拟机是在硬件层面实现虚拟化，然后运行一个完整的操作系统</li>
</ul>
<blockquote>
...
<h3 id="在后台的docker-windows和linux的区别" tabindex="-1"><a class="header-anchor" href="#在后台的docker-windows和linux的区别" aria-hidden="true">#</a> 在后台的Docker：Windows和Linux的区别</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Docker在Windows系统上安装是不同的。在安装时，Docker会创建一个基于Linux的虚拟机，叫做MobyLinux虚拟机，这个虚拟机是基于Alpine Linux的。Docker应用程序会连接到此虚拟机，你便可以开始创建具有必要操作组件的容器了。</p>
