...
<blockquote>
<p>docker compose will not be maintained in future versions, but it is still the easiest and most convenient way to organize docker compose deployments into a separate project https://github.com/openim-sigs/openim-docker to maintain.</p>
</blockquote>
<p><strong>1. Clone the project</strong></p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token comment"># choose what you need, We take branch 3.2 as an example</span>
...
If you don't know OpenIM's versioning policy, 📚Read our release policy: https://github.com/OpenIMSDK/Open-IM-Server/blob/main/docs/conversions/version.md</p>
</blockquote>
<p><strong>2. Configure the config file</strong></p>
<p>If you tried to get started quickly with <code v-pre>make demo</code>, then you know that our config file is generated by automation.</p>
<p>You can use <code v-pre>make init</code> to quickly initialize a configuration file</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>Then feel free to modify your current config file,  you can also modify <code v-pre>/scripts/install/environment.sh</code> document template, <code v-pre>make init</code> is essentially rendering <code v-pre>environment.sh</code> template, and then through the <code v-pre>make init</code> to automatically generate a new configuration.</p>
<p>If you only need to change the config file for a short time, or if you don't want to make any major changes in the future, you can modify the `.env file directly</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token assign-left variable"><span class="token environment constant">USER</span></span><span class="token operator">=</span>root 								<span class="token comment">#no need to modify</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3. Deploy and start</strong></p>
<blockquote>
<p><strong>Note</strong></p>
<p>You can deploy either directly with <code v-pre>make install</code> or with <code v-pre>docker compose up</code>, the logic is the same</p>
</blockquote>
...
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>$ <span class="token function">make</span> check
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Looking at the command line at this point, there are two items in the output, checking for the start of the component port that OpenIM depends on, and the start of the OpenIM core component</p>
</details> 
<details>  <summary>Compile from Source</summary>
<p>Ur need <code v-pre>Go 1.18</code> or higher version, and <code v-pre>make</code>.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>go version <span class="token operator">&amp;&amp;</span> <span class="token function">make</span> <span class="token parameter variable">--version</span> <span class="token operator">||</span> <span class="token builtin class-name">echo</span> <span class="token string">"Error: One of the commands failed."</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Version Details: https://github.com/OpenIMSDK/Open-IM-Server/blob/main/docs/conversions/version.md</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token comment"># choose what you need</span>
...
<p><code v-pre>make help</code> to help you see the instructions supported by OpenIM.</p>
</blockquote>
<p>Use <code v-pre>make check</code> to check all component starts</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>$ <span class="token function">make</span> check
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>You can use the <code v-pre>make help-all</code> see OpenIM in action.</p>
</details>
...
<p>The config file is available via <a href="https://github.com/OpenIMSDK/Open-IM-Server/blob/main/scripts/install/environment.sh" target="_blank" rel="noopener noreferrer">environment.sh<ExternalLinkIcon/></a> configuration <a href="https://github.com/OpenIMSDK/Open-IM-Server/blob/main/deployments/templates/openim.yaml" target="_blank" rel="noopener noreferrer">openim.yaml<ExternalLinkIcon/></a> template, and then through the <code v-pre>make init</code> to automatically generate a new configuration.</p>
<ul>
<li>
<p>Zookeeper</p>
<ul>
<li>
<p>Used for RPC service discovery and registration, cluster support.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>zookeeper:
...
<ul>
<li>
<p>Used for storing users, relationships, and groups, supports master-slave database.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>mysql:
...
<ul>
<li>
<p>Used for storing offline messages, supports mongo sharded clusters.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>mongo:
...
<ul>
<li>
<p>Used for storing message sequence numbers, latest messages, user tokens, and mysql cache, supports cluster deployment.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>redis:
...
<ul>
<li>
<p>Used for message queues, for message decoupling, supports cluster deployment.</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>kafka:
  username:                               <span class="token comment">#username</span>
  password:                               <span class="token comment">#password</span>
...
<p>read: https://github.com/OpenIMSDK/Open-IM-Server/blob/main/deployments/README.md</p>
</details> 
...
<tr>
<td>TCP:10001</td>
<td>ws protocol, message port such as message sending, pushing etc, used for client SDK</td>
<td>Port release or nginx reverse proxy, and firewall off</td>
</tr>
<tr>
<td>TCP:10002</td>
<td>api port, such as user, friend, group, message interfaces.</td>
<td>Port release or nginx reverse proxy, and firewall off</td>
</tr>
<tr>
<td>TCP:10005</td>
<td>Required when choosing minio storage (openIM uses minio storage by default)</td>
<td>Port release or nginx reverse proxy, and firewall off</td>
</tr>
</tbody>
</table>
...
<ul>
<li>chat warehouse: https://github.com/OpenIMSDK/chat</li>
</ul>
<table>
<thead>
<tr>
